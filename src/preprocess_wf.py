'''
Functions for preprocessing of waveforms.
...
Functions:
    ...
    * get_desired_shape()
        (wf, ts) --> (wf, ts)
    * apply_mean_ev_threshold()
        (wf, ts) --> (wf, ts)
    * apply_amplitude_thresh()
        (wf, ts) --> (wf, ts) OR:
        (wf, ts, labels) --> (wf, ts, labels).
...
def get_desired_shape(waveforms, timestamps, hypes, training=True):
    ...
    To achieve the desired number of dimensions/samples, each CAP is either cut-off at specified dimension,
    or extended with zeros for missing dimensions.
    ...
    training : Boolean
        This specifies which hypes-param to use for downsampling. \\
            True => hypes["preprocess"]["desired_num_of_samples"] \\
            False => hypes["preprocess_for_eval"]["desired_num_of_samples"]

    Returns
    -------
    (waveforms, timestamps)
        waveforms : (new_number_of_waveforms, new_dim_of_waveform) numpy_array

        timestamps : (new_number_of_waveforms, 1) numpy_array
    '''
    ...
    if timestamps[-1] > end_time*60:
        ...
        # No timestamp past the end-time..
...
def apply_mean_ev_threshold(waveforms, timestamps, mean_event_rates, hypes, ev_labels=None):
    '''
    Applies event-rate threshold to only consider "high occurance"-CAPs as specified by a set threshold.
    ...
    OBS!! The returned labels are Transposed.. Input-shape does not make sense.. \\
    Also, the shape of output timestamps is (n_wf, ), NOT (n_wf, 1).
    ...
    mean_event_rates : (n_wf,) array_like
            Estimated mean event rate during the considered recording time for all CAPs.
    hypes : .json file
            Containing hyperparameters
    ev_labels :  (3, n_wf) array_like
        label for each waveform.

    Returns: 
    --------
    if ev_labels is not ``None``:
        (high_occurance_wf, high_occurance_ts, high_occurance_ev_labels)
    else:
        (high_occurance_wf, high_occurance_ts)
    ...
    if ev_thresh_procentage:
        if relative_EV:
            ...
            N = waveforms.shape[0]             # Number of observed CAPs in total.
            mean_event_rate = N/T              # Total mean-event-rate
...
def apply_amplitude_thresh(waveforms, timestamps, hypes): 
    ...
    -------
    (waveforms, timestamps) : 
        Remaining waveforms/timestamps after applied threshold.
        waveforms : (n_wf_post_thresh, dim_of_waveform )
        timestamps :  (n_wf_post_thresh, 1 )
    '''
    # *** Extract hyperparameters from json file: **
