function [timestamps, waveforms, sig, thresh] = ...
...
% waveforms : waveforms of detected CAPs
...
Tcardiac = 0.005; % gg: width of wavelet for cardiac?? or prior on cardiac duration?? 
...
for k2 = 1:length(index_first_edge)
    ...
    index_range = index_range1; 
                  ...
                  % floor(index_first_edge(k2) + 2 * Npeak);
    % use sig (not y) because sig cleaner for peak alignment     
...
waveforms = waveforms(ia, :); % OUTPUTS WAVELET COEFFS ????
...
%    plot(t, thresh_sign(k)*thresh(k)*ones(size(t)), 'g')
