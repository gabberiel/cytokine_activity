...
% First cell is to convert .plx-files to .mat files. 
%
...
%% Load, concatinate, and save KI-DATA (using Marys electrode)
% Marys electrode have 16 channels (sometimes one is turned off due to errors..)
...
downsample = 1;
[channels, t] = convert_rhs_to_mat(file_dir_path, saveas_path, downsample);
...
% For KI-data, downsampling is done in "convert_rhs_to_mat()"
Nd = 1; % gg: only use every e.g 5th recorded datapoint.. 
...
%% Get data from multiple channels (KI-recordings) for plots and comparison.
clear all; clc; close all
...
for file = files'
    if startsWith(file.name, start_string_of_file)
        ...
        if count > 1
            all_wf(end+1) = cell2struct(repmat({[]},2,1),['wf'; 'ts']);
        else
            all_wf = cell2struct(repmat({[]},2,1),['wf'; 'ts']);
        end
        if first10000
            all_wf(count).wf = waveforms(1:10000,:);
            all_wf(count).ts = timestamps(1:10000);
        else
            all_wf(count).wf = waveforms;
            all_wf(count).ts = timestamps;
        end
        count = count + 1;
        disp(['Size of waveforms : ', num2str(size(waveforms))])
    end
