classdef NeuralRecording < handle
    properties
        ...
        datadec     % Gabe: Decomposed timeseries object
        dt          % delta time (s)
        dtdec       % Gabe: Decomposed dt
        sr          % sampling rate (Hz)
        srdec
        ...
        resp
        threshold   % timeseries object that stores thresholds
        Nd          % gg: Downsample parameter
        powerSpectrum
    ...
    methods
        ...
        function adaptive_threshold(obj, usewavelets, num_std, wdur, gdur, usedownsampled, alignmentmode, threshType, usetruncated)
            ...
            elseif usedownsampled && usetruncated
                ...
                disp('OBS, using original data to create waveforms..')
            ...
            % ------------- Gabriel comments ---------------------------
            % Signal Decomposition Algorithm: identify CAP occurence based on Wavelet
            % decomposition.
            % IN: y1 =  
            %
            % --RETURNING:
            % timestamps : occurence times of CAPs (in sec)
            % waveforms : waveforms of detected CAPs <-- qq: wavelet coeffs
            % ywm : wavelet coefficients
            % thresh : actual value of threshold applied
            %--------------------------------------------------------------------------
            [timestamps, waveforms_, obj.sig, thresh] = sig_decomp_new(t, y1, y2, threshType, num_std, usewavelets, wdur, gdur, alignmentmode);
            % if usewavelets then waveforms_ will be based on cwt
